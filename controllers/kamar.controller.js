...
exports.getAvalaible = async (req, res) => {
  ...
  let avalaible = await kamarModel.findAll({
    where: {
      [Op.and]: [{ id_tipe_kamar: id_tipe_kamar }, { status: "avalaible" }],
    },
  });
  return res.json({
    succsess: true,
...
exports.sumAvalaible = async (req, res) => {
  try {
    const avalaible = await sequelize.query(
      "SELECT tipe_kamars.nama_tipe_kamar, COUNT(kamars.nomor_kamar) AS avalaible_room FROM kamars INNER JOIN tipe_kamars ON tipe_kamars.id = kamars.id_tipe_kamar GROUP BY tipe_kamars.nama_tipe_kamar ORDER BY `avalaible_room` ASC"
    );
    return res.json({
      succsess: true,
      datas: avalaible[0],
